...
def write_graphs_from_list(graphs, graphs_indices, outfile):
    """
    Write NetworkX graphs from memory to file in FSG transaction format.
    Returns only the indices of graphs that are actually written.
    """
    graphs_indices_act = []
    with open(outfile, 'w') as fr:
        for ind in graphs_indices:
...
def novelty_from_list(graph_ref_list, graph_ref_indices,
                      graph_pred_list, graph_pred_indices,
                      temp_path, timeout):
    ...
    graph_pred_indices = write_graphs_from_list(graph_pred_list, graph_pred_indices, pred_path)
    graph_ref_indices = write_graphs_from_list(graph_ref_list, graph_ref_indices, test_path)
    ...
    print(f'{len(unique1)} / {len(calc1)} predicted graphs are not subgraphs of reference graphs')
    ...
    print(f'{len(unique2)} / {len(calc2)} predicted graphs do not have reference graphs as subgraphs')

    novel = len(set(unique1).intersection(unique2))
    total_eval = len(set(calc1).intersection(calc2))

    print(f'{novel} / {total_eval} graphs for sure novel')
    print('Novelty - {:.6f}'.format(novel / total_eval if total_eval else 0.0))
    ...
    return novel / total_eval if total_eval else 0.0

def uniqueness_from_list(graph_pred_list, graph_pred_indices,
                         temp_path, timeout):
    pred_fd, pred_path = tempfile.mkstemp(dir=temp_path)
    res_fd, res_path = tempfile.mkstemp(dir=temp_path)
    ...
    print(f'{len(unique1)} / {len(lines)} predicted graphs are unique')
    print('Uniqueness - {:.6f}'.format(len(unique1) / len(lines) if lines else 0.0))
    ...
    return len(unique1) / len(lines) if lines else 0.0
